...
struct Packet
{
    ...
    double   delay;        ///< 传播时延
    double   energy;       ///< 消耗的能量
    std::string  description;   ///描述这个数据包
    Packet():id(0),from(0),
...
extern std::default_random_engine e;          ///< 生成种子引擎
...
extern std::vector<std::vector<struct Packet>>          cachepacketVector;///< 每个节点等待发送的数据包
...
extern uint32_t   packets;           ///< 成功发包的个数
